    ...
    add_deco_badge(1, function () {
        add_deco_badge(2, function () {
            add_deco_badge(4, function () {
                $("#file_list_table_wrapper table").dataTable({
                    searching: false,
                    order: [[2, "desc"]]
                });
...
function add_deco_badge(model, callback) {
    $.get(API_SERVER + "tool/env/get/?key=model_" + model + "_active_" + Cookies.get('joker_id')).always(function (active) {
        $.get(API_SERVER + "model/" + model + "/source/", function (r) {
...
function show_edw_datasets() {
    $.get(API_SERVER + "connector/dataset-list/", function (r) {
        var html = "";
        for (var i = 0; i < r.length; i++) {
            html += "<p><i class='fa fa-folder-o font-blue'></i> " + r[i] + "</p>";
        }
        bootbox.alert({
            title: "Uploaded EDW Datasets",
            message: html
        });
    }).fail(function () {
        bootbox.hideAll();
        bootbox.alert(error_message("Cannot communicate with the core service server for listing EDW datasets!"));
    });
}

function set_active(model, source) {
    $.get(API_SERVER + "tool/env/set/?key=model_" + model + "_active_" + Cookies.get('joker_id') + "&value=" + source, function (r) {
        window.location.reload();
    }).fail(function () {
        bootbox.hideAll();
...
function clear_db(model, source) {
    ...
    $.get(API_SERVER + "model/" + model + "/delete_all/?source=" + source, function (r) {
        window.location.reload();
    }).fail(function () {
        bootbox.hideAll();
...
function interpret_data_type_desc(data_type) {
    var data_type_desc = {
        model_1: [
            ...
            "TURNOVER_RATIO", "ACTIVE_RATE_RATIO", "RECOVERY_RATE_RATIO", "AMPLIFICATION",
            ...
            "AGE_RC", "ACTIVE_RATE_RECENT", "TURNOVER_RATIO", "ACTIVE_RATE_RATIO",
    ...
    bootbox.dialog({
        ...
        buttons: {
            Proceed: function () {
                ...
                $.get(API_SERVER + "model/" + data_type + "/delete_all/?source=" + file, function (r) {
                    $.get(API_SERVER + "model/" + data_type + "/add_cust_from_csv/?src=" + file, function (r) {
                        var import_status = eval(r);
                        $.get(API_SERVER + "tool/env/set/?key=last_success_import&value=" + file, function (r) {
